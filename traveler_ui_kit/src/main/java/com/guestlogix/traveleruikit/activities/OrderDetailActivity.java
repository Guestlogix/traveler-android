...
import static com.guestlogix.travelercorekit.models.BookingPurchasedProduct.CANCELLATION_POLICY_NON_REFUNDABLE;
...
public class OrderDetailActivity extends AppCompatActivity implements CancellationQuoteCallback, EmailOrderConfirmationCallback {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        for (Product product : order.getProducts()) {
            ...
            if (product instanceof BookingPurchasedProduct) {
                BookingPurchasedProduct bookingPurchasedProduct = (BookingPurchasedProduct) product;
                String date = DateHelper.formatToMonthDayYear(bookingPurchasedProduct.getEventDate());
                String dateTime = date + "\n" + DateHelper.formatTime(bookingPurchasedProduct.getEventDate());

                productDateTextView.setText(dateTime);

                productPriceTextView.setText(bookingPurchasedProduct.getPrice().getLocalizedDescriptionInBaseCurrency());

                boolean isRefundable = !CANCELLATION_POLICY_NON_REFUNDABLE.equals(bookingPurchasedProduct.getCancellationPolicy());
                cancelButton.setEnabled(isRefundable);
                cancelButton.setText(isRefundable ? R.string.cancel_order : R.string.non_refundable);
            } else if(product instanceof PartnerOfferingPurchasedProduct)
            {
                PartnerOfferingPurchasedProduct partnerOfferingPurchasedProduct = (PartnerOfferingPurchasedProduct) product;

                productPriceTextView.setText(partnerOfferingPurchasedProduct.getPrice().getLocalizedDescriptionInBaseCurrency());

                productDateTextView.setText("");

                boolean isRefundable = !CANCELLATION_POLICY_NON_REFUNDABLE.equals(partnerOfferingPurchasedProduct.getCancellationPolicy());
                cancelButton.setEnabled(isRefundable);
                cancelButton.setText(isRefundable ? R.string.cancel_order : R.string.non_refundable);
            }
            ...
            productView.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    if(product instanceof PartnerOfferingPurchasedProduct)
                    {
                        return;
                    }
