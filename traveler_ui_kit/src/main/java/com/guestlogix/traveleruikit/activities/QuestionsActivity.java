...
public class QuestionsActivity extends AppCompatActivity {
    ...
    private BookingForm.BookingFormError focusedError;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (extras != null && extras.containsKey(EXTRA_BOOKING_FORM)) {
            ...
            if (bookingForm != null) {
                ...
                form.setOnFormClickListener(onFormClickListener);
                form.setOnFormValueChangedListener(onFormValueChangedListener);

                return;
            }
        }

        TravelerLog.e("PassSelectionActivity requires a BookingForm to operate.");
        finish();
    }
    ...
    Form.OnFormClickListener onFormClickListener = (sectionId, fieldId) -> {
        if (sectionId == bookingForm.getQuestionGroups().size()) {
            ...
            if (!errors.isEmpty()) {
                ...
                focusedError = errors.get(0);
                form.reload(focusedError.getGroupId(), focusedError.getQuestionId());
            } else {
                // Submit form.
            }
        }
    ...
    Form.OnFormValueChangedListener onFormValueChangedListener = (sectionId, fieldId, value) -> {
        Question question = bookingForm.getQuestionGroups().get(sectionId).getQuestions().get(fieldId);
        QuestionType type = question.getType();
        Answer answer = null;
        switch (type) {
